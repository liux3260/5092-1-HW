...
{
    class Lookback:getpayoff
    {

        public Lookback(Parameters GIP):base (GIP)
        {
        ...
        public double[] get_payoff()//get payoff
        {
            ...
            Action<object> po = x =>
            {
                ...
                for (int j = startc; j < endc; j++)
                {
                    ...
                    if (check2 == 1)//control variate
                    {
                        payoffc1[j] = Math.Max(S1max - strike, 0) + beta1 * cvc1;//get the payoff matrix
                        payoffp1[j] = Math.Max(strike - S1max, 0) + beta1 * cvp1;
                        if (check1 == 1)//control variate+antithetic
                        {
                            payoffc2[j] = Math.Max(S2max - strike, 0) + beta1 * cvc2;//get the payoff matrix
                            payoffp2[j] = Math.Max(strike - S2max, 0) + beta1 * cvp2;
                    ...
                    {
                        payoffc1[j] = Math.Max(S1max - strike, 0);//get the payoff matrix
                        payoffp1[j] = Math.Max(strike - S1max, 0);
                        if (check1 == 1)
                        {
                            payoffc2[j] = Math.Max(S2max - strike, 0);//get the payoff matrix
                            payoffp2[j] = Math.Max(strike - S2max, 0);
                            payoffc[j] = payoffc1[j] + payoffc2[j];
                            payoffp[j] = payoffp1[j] + payoffp2[j];
                        }
                        else
