...
{
    class Digital : getpayoff
    {
        public double rebate{get;set;}//Rebate is the payoff of the Digital option. It is 1 for the most of time.
        public Digital(Parameters GIP):base (GIP)
        {
        ...
        public double[] get_payoff()//get payoff
        {
            ...
            Action<object> po = x =>
            {
                ...
                for (int j = startc; j < endc; j++)
                {
                    ...
                    if (check2 == 1)//control variate
                    {
                        if (Form1.path1[j, simulation] - strike > 0)//different from European option
                        {
                            payoffc1[j] = rebate + beta1 * cvc1;//get the payoff matrix
                            payoffp1[j] = 0 + beta1 * cvp1;
                        }
                        else
                        {
                            payoffc1[j] = 0 + beta1 * cvc1;
                            payoffp1[j] = rebate+beta1 * cvp1;
                        }

                        if (check1 == 1)//control variate+antithetic
                        {
                            if (Form1.path2[j, simulation] - strike > 0)
                            {
                                payoffc2[j] = rebate + beta1 * cvc2;//get the payoff matrix
                                payoffp2[j] = 0 + beta1 * cvp2;
                            }
                            else
                            {
                                payoffc2[j] = 0 + beta1 * cvc2;
                                payoffp2[j] = rebate + beta1 * cvp2;
                            }
                    ...
                    {
                        if (Form1.path1[j, simulation] - strike > 0)
                        {
                            payoffc1[j] = rebate;//get the payoff matrix
                            payoffp1[j] = 0;
                        }
                        else
                        {
                        ...
                        if (check1 == 1)
                        {
                            if (Form1.path2[j, simulation] - strike > 0)
                            {
                                payoffc2[j] = rebate;//get the payoff matrix
                                payoffp2[j] = 0;
                            }
                            else
                            {
