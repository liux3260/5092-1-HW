...
{
    class Range:getpayoff
    {
        public Range(Parameters GIP):base (GIP)
        {
            ...
            Action<object> po = x =>
            {
                ...
                for (int j = startc; j < endc; j++)
                {
                    ...
                    for (int i = 0; i < simulation; i++)
                    {
                        ...
                        if (Form1.path1[j, i+1] > S1max)//change from European option
                        {
                    ...
                    if (check2 == 1)//control variate
                    {
                        payoffc1[j] = S1max - S1min + beta1 * cvc1;//get the payoff matrix
                        payoffp1[j] = payoffc1[j];
                        if (check1 == 1)//control variate+antithetic
                        {
                            payoffc2[j] = S2max - S2min + beta1 * cvc2;//get the payoff matrix
                            payoffp2[j] = payoffc2[j];
                    ...
                    {
                        payoffc1[j] = S1max - S1min;//get the payoff matrix
                        payoffp1[j] = payoffc1[j];
                        if (check1 == 1)
                        {
                            payoffc2[j] = S2max - S2min;//get the payoff matrix
                            payoffp2[j] = payoffc2[j];
                            payoffc[j] = payoffc1[j] + payoffc2[j];
                            payoffp[j] = payoffp1[j] + payoffp2[j];
                        }
